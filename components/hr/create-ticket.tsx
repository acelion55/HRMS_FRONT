...
export function CreateTicketForm({ onClose, onCreated, template }: Props) {
  const { currentUser } = useAuth()
  const [title, setTitle] = useState(template?.title || "")
  const [description, setDescription] = useState(template?.content || "")
  const [category, setCategory] = useState<TicketCategory>(template?.category as TicketCategory || "GENERAL")
  const [priority, setPriority] = useState<TicketPriority>(template?.priority as TicketPriority || "MEDIUM")
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      remoteTicket = await createTicketOnDb(payload)
    } catch (error) {
      console.error("create-ticket API failed", error)
      toast.error("Saved locally, but unable to sync with DB")
    }
    ...
    const auditEntry = {
      ...
      ticketId: (remoteTicket?.id ?? localTicket.id),
    ...
    void addAuditLogOnDb(auditEntry)

    toast.success("Ticket created!", { description: localTicket.title })
    onCreated(localTicket)
    onClose()
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="w-full max-w-lg bg-white rounded-2xl shadow-2xl max-h-[90vh] flex flex-col border border-gray-100">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4 overflow-y-auto flex-1">
          ...
          {/* Assign To — employees see HR users, HR roles see everyone */}
          {(showAssign || user.role === "EMPLOYEE") && (
            <div>
              <label className="flex items-center gap-1.5 text-xs font-bold text-gray-600 uppercase tracking-wide mb-1.5">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    assigneeId === ""
                      ...
                      : "border-gray-200 text-gray-500 hover:border-yellow-300 bg-white"
                  )}
                >
                  <div className="h-6 w-6 rounded-full bg-gray-200 flex items-center justify-center text-xs font-bold text-gray-500 shrink-0">?</div>
                ...
                {assignableUsers.map((u) => (
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      <p className="text-xs font-semibold text-gray-800 truncate">{u.name.split(" ")[0]}</p>

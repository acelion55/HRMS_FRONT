...
export async function createTicketOnDb(ticketData: Omit<Ticket, "id" | "createdAt" | "updatedAt" | "deletedAt" | "comments">) {
  try {
    ...
    console.error("createTicketOnDb failed", error)
    return null
  }
}

export async function addAuditLogOnDb(log: Omit<AuditLog, "id">) {
  try {
    return await safeFetchJson("/api/audit-logs", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    console.error("addAuditLogOnDb failed", error)
    return null
  }
}

export async function patchTicketOnDb(ticketId: string, update: Record<string, unknown>) {
  try {
    return await safeFetchJson(`/api/tickets/${ticketId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(update),
    })
  } catch (error) {
    console.error("patchTicketOnDb failed", error)
    return null
  }
}
...
export function addAuditLog(log: Omit<AuditLog, "id">) {
  ...
  void addAuditLogOnDb(entry).catch(() => {
    // ignore in background if remote fails
  })
...
export function updateTicketStatus(ticketId: string, status: Ticket["status"]) {
  ...
  void patchTicketOnDb(ticketId, { status })
...
export function assignTicket(ticketId: string, assigneeId: string, assigneeName: string) {
  ...
  void patchTicketOnDb(ticketId, { assigneeId, assigneeName })
  return tickets.find((t) => t.id === ticketId)!
}
...
export function softDeleteTicket(ticketId: string) {
  ...
  void patchTicketOnDb(ticketId, { deletedAt: new Date().toISOString() })
}
...
export function addComment(ticketId: string, comment: Omit<TicketComment, "id">) {
  ...
  void patchTicketOnDb(ticketId, { comment: newComment })
...
export function checkAndEscalateTickets() {
  ...
  tickets = tickets.map((ticket) => {
    if (ticket.deletedAt || ticket.status === "CLOSED" || ticket.status === "RESOLVED") {
      return ticket
    }
    ...
    // Escalate based on time open
    if (daysOpen >= 7 && ticket.priority === "LOW") {
      newPriority = "MEDIUM"
      escalatedFlag = true
    } else if (daysOpen >= 14 && ticket.priority === "MEDIUM") {
      newPriority = "HIGH"
      escalatedFlag = true
    } else if (daysOpen >= 21 && ticket.priority === "HIGH") {
      newPriority = "CRITICAL"
      escalatedFlag = true
    }

    if (escalatedFlag) {
      const updatedTicket = { ...ticket, priority: newPriority, updatedAt: now.toISOString() }
